    ...
    , selectedPilot(player.Identifier())
{
    ...
    if (player.GetPlanet() && !player.IsDead() && !gamePanels.IsTop(&*gamePanels.Root()) && gamePanels.CanSave())
        player.Save();
    UpdateLists();
}
...
bool LoadPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command& command, bool isNewPress)
{
    if (key == 'n')
    {
        ...
        ConversationPanel* panel = new ConversationPanel(player, *GameData::Conversations().Get("intro"));
        GetUI()->Push(panel);
    ...
    else if (key == 'D' && !selectedPilot.empty())
    {
        GetUI()->Push(new Dialog(
            this, &LoadPanel::DeletePilot,
            "Are you sure you want to delete the selected pilot, \"" + selectedPilot + "\", and all their saved games?"));
    }
    else if (key == 'a' && !player.IsDead() && player.IsLoaded())
    {
        ...
        GetUI()->Push(new Dialog(
            this, &LoadPanel::SnapshotCallback,
...
void LoadPanel::UpdateLists()
{
    ...
    for (auto& it : files)
        sort(it.second.begin(), it.second.end(), [](const pair<string, time_t>& a, const pair<string, time_t>& b) -> bool {
...
void LoadPanel::SnapshotCallback(const string& name)
{
    ...
    if (Files::Exists(to))
    {
        ...
        EM_ASM(FS.syncfs(function(err) {
            assert(!err);
            console.log("IndexedDB updated 2!");
        }););
#endif
    }
    else
